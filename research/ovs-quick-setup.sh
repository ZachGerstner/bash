...
ping -c 3 $PARTNER_IP > ping.log
CONNECTION=$(cat ping.log | awk '{print$1}' | grep "From")
if [ -z $CONNECTION ]
	echo 'Connection check failed!'
	echo 'Check network for proper connectivity.'
	exit
else	
echo 'Connection seems to be working!'
